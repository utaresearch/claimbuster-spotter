...
def main():
    ...
    dataset_train = tf.data.Dataset.from_tensor_slices(([list(x[0]) for x in train_data.x], train_data.y)).shuffle(
        buffer_size=train_data.get_length()).batch(FLAGS.batch_size_reg if not FLAGS.adv_train else FLAGS.batch_size_adv)
    dataset_test = tf.data.Dataset.from_tensor_slices(([list(x[0]) for x in test_data.x], test_data.y)).shuffle(
